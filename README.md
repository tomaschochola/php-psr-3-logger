# PHP PSR-3: OTLP Logger Implementation

This library provides a strictly typed, zero-overhead implementation of the PSR-3 Logger Interface, architected specifically for OpenTelemetry (OTLP) integration. Unlike traditional loggers that write to disk, this implementation treats logs as structured events designed for immediate export to observability platforms.

The logger is built to minimize blocking operations within the request cycle. It formats log records into strictly typed JSON structures and leverages compliant HTTP clients for transport, ensuring that observability does not compromise the performance stability of the host application.
